...
description: "Web applications and systems over the internet communicate via APIs. The data exchanged can be in any form, one of those 
forms is a standard architecture called REST. In this article, you'll learn what a RESTful  API is and how it's used to build decouple web
...
This approach consists mainly of two parts in web application — the [frontend](https://en.wikipedia.org/wiki/Front-end_web_development) and the [backend](https://en.wikipedia.org/wiki/Front-end_web_development) — that are separated from each other. The backend deals only with the server and database(s) and provides an API through *endpoints* for the frontend or client to consume. The front-end then manages the presentation (the part users see) of that data. 
